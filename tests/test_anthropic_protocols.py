...
def test_native_body_lifts_system_and_enables_thinking():
    ...
    assert body["tools"] == [{"name": "read", "description": "Read", "input_schema": {"type": "object"}}]
...
def test_signed_thinking_and_provider_tool_id_survive_tool_continuation():
    ...
    system, history = chat_to_anthropic([
        {"role": "system", "content": "S"}, assistant,
...
def test_stream_assembly_retains_thinking_signature_and_tool_input():
    result = assemble_anthropic_stream([
        ...
        {"type": "content_block_start", "index": 1, "content_block": {"type": "tool_use", "id": "p1", "name": "read", "input": {}}},
        {"type": "content_block_delta", "index": 1, "delta": {"type": "input_json_delta", "partial_json": json.dumps({"path": "x"})}},
        {"type": "message_delta", "usage": {"input_tokens": 4, "output_tokens": 5}},
    ])
    message = result["choices"][0]["message"]
    assert message["_anthropic_content"][0] == {"type": "thinking", "thinking": "reason", "signature": "sig"}
