...
def _coerce_reasoning_effort(model: str, effort: str | None) -> str | None:
    ...
    if name.startswith("glm-"):
        return "high" if effort == "medium" else effort
    if name.startswith("deepseek-") or name.startswith("deepseek_"):
        ...
        if effort in ("high", "max"):
            return effort
        return None
    # OpenAI-family reasoning models accept low/medium/high.
...
def _sanitize_messages(
    ...
    thinking can be replayed. DeepSeek needs reasoning passed back only on
    assistant turns that carried tool calls (the API requires it then); it is
    dropped on tool-call-free turns to save tokens and protect cache prefixes.
    """
    ...
    for original in messages:
        ...
        if role == "assistant":
            ...
            keep_reasoning = is_deepseek and has_tool_calls
...
def _build_request(
    ...
    *,
    ...
    sanitized_messages = _sanitize_messages(normalized_messages, model, is_anthropic)
